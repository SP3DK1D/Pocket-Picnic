...
{
    /// <summary>
    /// Simple attention-grabber for UI: pulses **scale**, **alpha**, or both.
    /// Uses **unscaled time** by default so it keeps pulsing on menus/pauses.
    /// 
    /// It pauses the effect while hovered/pressed (configurable) so buttons
    /// still feel responsive and don't fight the user's input.
    /// 
    /// Implementation notes:
    /// - All component lookups are cached in Awake (no per-frame allocations).
    /// - OnDisable restores baseline scale/alpha to avoid lingering visuals.
    /// </summary>
        ...
        IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        [Tooltip("Pause the pulse when the pointer hovers the control.")]
        public bool pauseOnHover = true;

        [Tooltip("Pause the pulse while the control is pressed.")]
        ...
        {
            _t = 0f;
            // Guard against NaN inputs that could creep in from bad Inspector values.
            scaleMin = Mathf.Max(0.0001f, scaleMin);
        ...
        private void Update()
        {
            ...
            // 0..1 wave based on sine, smooth and allocation-free
