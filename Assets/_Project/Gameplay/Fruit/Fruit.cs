...
{
    /// <summary>
    /// A single falling fruit instance.
    /// Optimizations:
    /// - Per-frame Camera.main cache shared by all fruits
    /// - Static property reads (powerups) pulled to locals each frame
    /// - isVisible fast path; minimal viewport math fallback
    /// - Allocation-free Update/LateUpdate
    /// </summary>
    [RequireComponent(typeof(SpriteRenderer))]
    [RequireComponent(typeof(Collider2D))]
    public class Fruit : MonoBehaviour
    {
        // Global live registry (unchanged, used by spawners/clear powerup)
        ...
        // convenience props (read-only, used by BasketCatchZone)
        ...
        // Per-frame camera cache for ALL fruits this frame (saves Camera.main cost)
        static class CamCache
        {
            static int _frame = -1;
            static Camera _main;
            public static Camera Main
            {
                get
                {
                    ...
                    if (f != _frame)
                    {
                        _main = Camera.main; // one lookup per frame
                        _frame = f;
                    }
                    return _main;
                }
        ...
        /// <summary>Initializer used by spawner/pool.</summary>
        ...
        public void Init(FruitData fd, float speedMultiplier, float groundY, bool decorative)
        {
            ...
            // Random tumble (+% via DifficultyManager)
            _tumbleDir = (URandom.value < 0.5f) ? -1 : 1;
            float baseT = URandom.Range(35f, 90f);
            float tMul = Mathf.Max(0f, DifficultyManager.TumbleMultiplier); // default 1.4
            _tumbleSpeed = baseT * (tMul <= 0f ? 1f : tMul);
        ...
        void Update()
        {
            // Pull powerup flags to locals (static property reads are cheap but this is cleaner)
            float freezeMul = PowerupManager.FreezeSpeedMul;   // 1 when not freezing
            bool magnetOn = PowerupManager.MagnetActive;
            float magnetR = PowerupManager.MagnetRadius;
            float magnetSpeed = PowerupManager.MagnetPullSpeed;
            Transform playerT = PowerupManager.PlayerTransform;

            // Vertical fall (Freeze scales gravity, fruit uses that multiplier)
            ...
            if (!decorative && data != null && !data.isBomb && magnetOn && playerT)
            {
                Vector3 to = playerT.position - transform.position;
                float dist = to.magnitude;
                if (dist <= magnetR && dist > 0.001f)
                {
                    float closeness = 1f - Mathf.Clamp01(dist / magnetR); // 0 far..1 near
                    float speed = magnetSpeed * (0.4f + 0.6f * closeness);
        ...
        {
            // Fast path via renderer. If we have a SR and it's visible to any camera, we're visible.
            if (_sr && _sr.isVisible) return true;

            // Fallback: do a minimal viewport check vs. the main camera (cached)
            var cam = CamCache.Main;
            if (!cam) return true; // assume visible if no camera context (editor boot edge case)
            Vector3 vp = cam.WorldToViewportPoint(transform.position);
            // Small padding (±5%) so we don't flap visibility right on the edge
        ...
        public void Retire()
        {
            // Avoid double work if already inactive (can happen if two systems target it the same frame)
